...
Context :: struct {
    dirty_epoch:     u64,
    last_draw_epoch: u64,
    keys:            bit_set[enum {Left, Right, Up, Down, Enter, Escape}},
    mouse_pos:       [2]f32,
    current_focus:   Id,
    active_id:       Id,
    cursor:          [2]f32,
...
push_draw_command :: proc(kind: enum {Button, SliderFill, SliderTrack, ProgressBar}, id: Id, bounds: [4]f32, data: rawptr = nil) {
    // Stub - backend will implement rendering
}
...
button :: proc(label: string, id: Id = 0) -> bool {
    ...
    if ctx.dirty_epoch != ctx.last_draw_epoch || hovered {
        push_draw_command(.Button, uid, bounds)
    }
...
slider :: proc(value: ^$T, min, max: T, label := "", id: Id = 0) -> bool where intrinsics.type_is_numeric(T) {
    ...
    bounds := layout_allocate(200, 20)
    if ctx.dirty_epoch != ctx.last_draw_epoch {
        push_draw_command(.SliderTrack, uid, bounds)
        fill := bounds
        fill.z *= f32(value^ - min) / f32(max - min)
        push_draw_command(.SliderFill, uid, fill)
    }
...
progress_bar :: proc(value: ^f32, min, max: f32) {
    ...
    if ctx.dirty_epoch != ctx.last_draw_epoch {
        push_draw_command(.ProgressBar, uid, bounds, value)
    }
}
